...
from sklearn.ensemble import RandomForestRegressor # Hoặc model của bạn
from constants import PATHS, FEATURES, TARGETS
from processor import TikTokDataProcessor

def main():
    # 1. Load & Process
    print("--- Đang tải dữ liệu và xử lý feature ---")
    ...
    for target in TARGETS:
        if target in train_df.columns:
            ...
            results[f"{target}_accuracy_%"] = (1 - abs(y_actual - y_pred) / (y_actual + 1)) * 100

    # 4. Save Results
    result_df = pd.DataFrame(results)
    result_df.to_csv(PATHS["output_result"], index=False, encoding="utf-8-sig")
    print(f"Thành công! File kết quả tại: {PATHS['output_result']}")
